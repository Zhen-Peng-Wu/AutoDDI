...
from set_config import data_name
import os



df_drugs_smiles = pd.read_csv(os.path.join('dataset', data_name, 'drug_smiles.csv'))
...
df_all_pos_ddi = pd.read_csv(os.path.join('dataset', data_name, 'ddis.csv'))
...
class DrugDataset(Dataset):
    def __init__(self, tri_list, ratio=1.0,  neg_ent=1, disjoint_split=True, shuffle=True):
        ...
        if len(tri_list[0]) == 3:
            for h, t, r, *_ in tri_list:
                if ((h in MOL_EDGE_LIST_FEAT_MTX) and (t in MOL_EDGE_LIST_FEAT_MTX)):
                    ...
                    self.rel_total = max(self.rel_total, r+1)
        ### same as the previous methods, like DSN-DDI
        elif len(tri_list[0]) == 4:
        ...
        self.n_atom_feats = MOL_EDGE_LIST_FEAT_MTX[self.tri_list[0][0]][1].shape[1]
    ...
    def collate_fn(self, batch):
        ...
        ### twosides, same as the previous methods, like DSN-DDI
        elif len(batch[0]) == 4:
