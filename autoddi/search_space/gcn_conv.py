...
class GCNConv(MessagePassing):
    ...
    def forward(self, x: Union[Tensor, OptPairTensor], edge_index: Adj,
        ...
        if isinstance(x, Tensor):
            ...
            if self.normalize:
                ...
                # Use the degree of the target node as a weight to aggregate the features of the source node
                edge_weight = 1.0 / deg[col]
    ...
    def message_and_aggregate(self, adj_t: SparseTensor, x: Union[Tensor, OptPairTensor]) -> Tensor:
        if isinstance(x, Tensor):
